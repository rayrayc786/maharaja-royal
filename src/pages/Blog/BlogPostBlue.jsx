...
export const BlogPostBlue = () => {
  ...
  if (!post) {
    return (
      <div className="min-height-screen bg-royal-gradient text-cream flex flex-col justify-center items-center p-8">
        <h2 className="font-serif text-3xl mb-4">Article Not Found</h2>
        <Link to="/blog-1" className="px-6 py-3 bg-gold text-royal-blue font-medium rounded-full hover:bg-cream transition-colors duration-300">
          Back to Journal
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-height-screen bg-royal-gradient text-cream pt-32 pb-24">
      {/* Article Header Container */}
      <div className="max-w-[1000px] mx-auto px-6 md:px-12">
        <button 
          onClick={() => navigate('/blog-1')}
          className="inline-flex items-center text-xs tracking-widest uppercase font-semibold text-gold hover:text-cream transition-colors duration-300 mb-8"
        >
        ...
        <div className="article-header-element flex flex-wrap gap-4 items-center text-xs text-gold tracking-widest uppercase mb-4">
          ...
          <span className="text-cream/60">•</span>
          <span className="text-cream/60">{post.readTime}</span>
        </div>

        {/* Title */}
        <h1 className="article-header-element font-serif text-[clamp(2rem,5vw,4.5rem)] leading-[1.1] text-cream mb-6">
          {post.title}
        </h1>

        {/* Author info */}
        <div className="article-header-element flex items-center gap-3 mb-10 pb-8 border-b border-cream/10">
          ...
          <div>
            <p className="text-sm font-medium text-cream">{post.author}</p>
            <p className="text-[0.7rem] uppercase tracking-wider text-cream/50">Culinary Team</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 mb-12">
        <div className="aspect-[21/9] w-full overflow-hidden rounded-2xl border border-cream/10 relative">
          <img 
            src={post.image} 
            alt={post.title} 
      ...
      <article className="max-w-[800px] mx-auto px-6 md:px-12 mb-20">
        <div className="flex flex-col gap-6 text-cream/95 font-sans font-light text-base md:text-lg leading-relaxed">
      ...
      <div className="max-w-[1200px] mx-auto px-6 md:px-12 pt-16 border-t border-cream/10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {relatedPosts.map((item) => (
            <Link 
              key={item.id} 
              to={`/blog-1/${item.id}`} 
              className="group block relative overflow-hidden rounded-xl border border-cream/10 bg-[#0A1930] h-full flex flex-col justify-between"
            >
              ...
              <div className="p-5 flex-1 flex flex-col justify-between">
                <div>
                  <div className="flex gap-2 text-[0.65rem] text-cream/50 mb-1.5">
                    <span>{item.date}</span>
                    <span>•</span>
                    <span>{item.readTime}</span>
                  </div>
                  <h4 className="font-serif text-base text-cream group-hover:text-gold transition-colors duration-300 line-clamp-2 leading-snug">
                    {item.title}
                  </h4>
                </div>
