...
export const GalleryBlue = () => {
  ...
  return (
    <div className="min-height-screen bg-royal-gradient relative text-cream pt-32 pb-24 px-6 md:px-12 lg:px-24 overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <div className="max-w-[1600px] mx-auto mb-12 text-center">
          <span className="text-[0.8rem] uppercase tracking-[0.3em] text-gold mb-3 block">Visual Feast (Blue Variant)</span>
          <SplitTextReveal 
            className="font-serif text-[clamp(2.5rem,5vw,5rem)] leading-[1.1] text-cream mb-4" 
            text="The Royal Gallery" 
          />
          <p className="font-sans font-light text-cream/70 text-lg max-w-[600px] mx-auto mt-4 leading-relaxed">
          ...
          {[
            ...
            <button
              ...
              className={`px-6 py-2.5 rounded-full text-xs font-semibold tracking-wider uppercase border transition-all duration-300 ${
                activeFilter === tab.id
                  ? 'bg-cream text-royal-blue border-cream shadow-md'
                  : 'bg-[#0A1930] text-cream border-cream/15 hover:border-cream/60'
              }`}
            >
              {tab.label}
            </button>
        ...
        <div className="max-w-[1600px] mx-auto columns-1 md:columns-2 lg:columns-3 xl:columns-4 gap-6 space-y-6">
          {filteredImages.map((item, index) => (
            <div 
              key={item.id} 
              className="gallery-item break-inside-avoid overflow-hidden rounded-xl border border-cream/10 bg-[#0A1930] group cursor-pointer shadow-sm hover:shadow-lg transition-all duration-300"
            ...
            >
              <div className="relative overflow-hidden">
                <img 
                  ...
                  loading="lazy"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-[#0A1930] via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6">
                  <span className="text-[0.65rem] uppercase tracking-widest text-gold font-medium mb-1">{item.category.replace('-', ' & ')}</span>
                  <h4 className="text-cream font-serif text-lg leading-snug">{item.title}</h4>
                  <p className="text-cream/70 font-sans font-light text-xs mt-1">{item.description}</p>
                </div>
              </div>
            </div>
          ))}
